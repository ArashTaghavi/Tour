    ...
    <card :title=$route.name :active_loading=false>
        ...
        <div class="row" v-if="gallery.images!=null">
            <div class="table-responsive">
                <div class="section-title">گالری هتل</div>
...
<script>
    export default {
        ...
        methods: {
            handleSubmit() {
                axios.post(`/image-hostel-rooms/${this.$route.params.id}`, this.form).then(response => {
            ...
            getGallery() {
                axios.get(`/image-hostel-rooms/${this.$route.params.id}`)
                    .then(response => this.gallery = response.data)
                    .catch(error => this.errorNotify(error));
            },
            handleDelete(id) {
                axios.delete(`/image-hostel-rooms/${id}`)
                    .then(response => {
                        this.successNotify(response);
                        this.getGallery();
                    })
